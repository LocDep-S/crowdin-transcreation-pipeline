...
2. **The `string.status_on_step.recalculation_triggered` payload shape**
   ...
   once confirmed.
3. **Workflow step port names** (`manifest.json`). `"initial"`/`"untranslated"`
   input and `"translated"`/`"fallback"` output are placeholders — confirm
   against the real workflow editor once this step is actually inserted.
   Routing model: this step sits FIRST, ahead of AI Pre-translation.
   `"translated"` (success) routes straight to Proofreading, skipping the
   rest of the chain. `"fallback"` (failure/untranslated) routes into the
   existing AI Pre-translation step, so a miss still gets a normal shot at
   translation via the standard chain rather than being parked.
...
7. **Failure-routing port** on pipeline errors — routes to `"fallback"`,
   which the workflow editor wires into the existing AI Pre-translation
   step rather than parking the string. This was an open decision in the
   plan; it's now settled this way.
