...
async function processRecalculationEvent(event) {
  ...
  if (!crowdinApi.stringHasTranscreationLabel(stringObj)) {
    console.warn(`[webhook] stringId=${stringId} reached the transcreation step without the Transcreation label - routing via the fallback port (into the normal AI Pre-translation chain) without calling the pipeline.`);
    await crowdinApi.reportWorkflowStepOutput(accessToken, domain, projectId, workflowStepId, languageId, stringId, "fallback");
    return;
  }
  ...
  try {
    ...
    // Failure-routing (decided): route to the "fallback" port, which the
    // workflow editor wires into the existing AI Pre-translation step, so a
    // failed/untranslated string still gets a normal shot at translation via
    // the standard chain instead of being parked.
    await crowdinApi.reportWorkflowStepOutput(accessToken, domain, projectId, workflowStepId, languageId, stringId, "fallback").catch(() => {});
  }
}
