 ...
 * last wins, silently). A 5-minute TTL is generous for one full pipeline run
 * (audit/data/geo in parallel, then brief, then writer) while still
 * self-healing if a process crashes mid-run without releasing the lock.
 */
const FILE_LOCK_PREFIX = "transcreation-pipeline:filelock:";
const FILE_LOCK_TTL_SECONDS = 5 * 60;
