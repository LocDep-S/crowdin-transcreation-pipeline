 ...
 *
 * All AI calls route through lib/gemini.js, a Vertex AI Gemini client
 * authenticated via a GCP service account (see that file's header for why -
 * Crowdin's own AI Prompt Completion endpoint doesn't support the freeform
 * multi-stage reasoning this pipeline needs, only translate/QA-shaped
 * prompts).
 *
 * `ctx` (first argument to every exported stage function) is currently
 * unused by the AI calls themselves (no Crowdin credentials needed to call
 * Vertex AI directly) but is kept as the first parameter on every stage
 * function for a consistent call signature and in case a future stage needs
 * to fetch something from Crowdin mid-pipeline - callers (routes/webhook.js,
 * routes/regenerate.js) still pass { accessToken, domain }.
 */

const { completeJson } = require("./gemini");
...
async function culturalAudit(ctx, { sourceText, targetLanguage, contentFormat = "blog post" }) {
  ...
  return completeJson({ system, prompt });
}
